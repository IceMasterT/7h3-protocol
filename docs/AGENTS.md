...
- Single test file: `npm run test -- src/gluv/language.test.ts`
...
- `src/gluv/language.ts` is the core implementation (tokenize/compile/run/assemble/disassemble + stdlib preload + demos).
- Protocol stack is split by responsibility:
  - `src/gluv/protocol.ts`: envelope model, canonicalization, validation, HMAC sign/verify
  - `src/gluv/protocolTransport.ts`: decode/encode (JSON + compact wire), inbound verification, replay checks, batch/session helpers
  - `src/gluv/protocolAgent.ts`: session abstraction and auto-response loop
  - `src/gluv/agentAdapter.ts`: drop-in raw-message adapter for AI framework integration
  - `src/gluv/frameworkAdapters.ts`: framework bridge helpers (LangChain/LlamaIndex message mapping + JSON-RPC MCP-style bridge)
  - `src/gluv/mcpGateway.ts`: line-based JSON-RPC gateway runtime over signed AIP internals
  - `src/gluv/index.ts`: public protocol/adapter export surface for external integrations
  - `src/gluv/protocolDemo.ts`: deterministic demo transcript used by UI/tests
- Benchmark harnesses:
...
- Tests live in `src/gluv/*.test.ts` plus CLI process tests in `scripts/*.test.ts` (no custom Vitest config/setup file).
...
- `use <library>` directives are expanded before parsing; available libs are hardcoded in `STANDARD_LIBRARIES` inside `src/gluv/language.ts`.
...
- Use `DistributedReplayCache` from `src/gluv/protocolReplay.ts` for multi-node replay defense (shared store reserve semantics).
...
- HMAC key import is cached (bounded map) in `src/gluv/protocol.ts`; preserve cache behavior for high-throughput signing/verification.
